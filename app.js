...
let labelPrintRunning = false;
let printServiceWarned = false;
let printServiceErrorNotified = false;
let sessionHydrating = Boolean(supabaseClient);
...
const els = {
  ...
  studentUseGuardianDataField: document.getElementById("studentUseGuardianDataField"),
  studentUseGuardianData: document.getElementById("studentUseGuardianData"),
...
function bindEvents() {
  ...
  if (els.studentUseGuardianData) {
    els.studentUseGuardianData.addEventListener("change", () => {
      handleStudentUseGuardianDataChange();
    });
  }
  if (els.studentGuardian) {
    els.studentGuardian.addEventListener("blur", () => {
      if (els.studentUseGuardianData?.checked) {
        handleStudentUseGuardianDataChange();
      }
    });
  }
...
function renderSession() {
  if (sessionHydrating) {
    els.sessionRole.textContent = "Conectando...";
...
async function hydrateFromSupabase() {
  ...
  try {
    ...
    state.session = { id: profile.id, name: formatPersonName(profile.name), role: profile.role };
...
async function ensureProfileAfterEmailConfirmation(user) {
  ...
  const payload = {
    id: user.id,
    name: formatPersonName(metadata.full_name || user.email || "Usuario"),
    role,
...
async function fetchStudents() {
  ...
  state.students = rows.map((student) => ({
    ...
    guardian: formatPersonName(student.primary_guardian_name),
    otherGuardians: "",
    phone: student.phone,
    address: student.address,
    notes: student.notes || "",
    owner: formatPersonName(student.primary_guardian_name || ""),
...
async function handleSignupSubmit(event) {
  ...
  const name = formatPersonName(els.signupName.value.trim());
...
function openStudentDialog(student) {
  ...
  if (els.studentUseGuardianData) {
    els.studentUseGuardianData.checked = false;
  }
  ...
  if (els.studentUseGuardianDataField) {
    els.studentUseGuardianDataField.style.display = isResponsavel ? "none" : "flex";
  }
...
async function saveStudent(event) {
  event.preventDefault();
  const isResponsavel = state.session?.role === "responsavel" && !isAdmin() && !isEquipe();
  const guardianName = formatPersonName(isResponsavel ? state.session?.name || "" : els.studentGuardian.value.trim());
  const ownerName = formatPersonName(isAdmin() || isEquipe() ? guardianName : state.session?.name || guardianName);
  const isVisitor = isResponsavel ? false : Boolean(els.studentIsVisitor.checked);
  let phoneValue = isResponsavel ? "-" : els.studentPhone.value.trim();
  let addressValue = isResponsavel ? "-" : els.studentAddress.value.trim();
  const useGuardianData = isResponsavel || Boolean(els.studentUseGuardianData?.checked);
  if (useGuardianData && guardianName) {
    const contact = await resolveGuardianContactData(guardianName);
    if (contact?.phone) {
    ...
    if (!contact?.phone && !contact?.address && !isResponsavel) {
      alert("Nao foi possivel localizar telefone/endereco do responsavel selecionado.");
      return;
    }
  }
  ...
  const payload = {
    ...
    name: formatPersonName(els.studentName.value.trim()),
    birth: els.studentBirth.value,
    className: getClassForBirth(els.studentBirth.value),
    guardian: guardianName,
    otherGuardians: isResponsavel ? "" : formatPersonName(els.studentOther.value.trim()),
    phone: phoneValue,
...
async function handleStudentUseGuardianDataChange() {
  if (!els.studentUseGuardianData?.checked) {
    return;
  }
  const isResponsavel = state.session?.role === "responsavel" && !isAdmin() && !isEquipe();
  ...
  const contact = await resolveGuardianContactData(guardianName);
  if (contact?.phone) {
    els.studentPhone.value = contact.phone;
  }
  if (contact?.address) {
    els.studentAddress.value = contact.address;
  }
}

async function resolveGuardianContactData(guardianName) {
  ...
  if (supabaseClient) {
    ...
    const { data: studentData } = await supabaseClient
      ...
      .ilike("primary_guardian_name", normalizedName)
      .limit(1);
    if (studentData?.length) {
      const row = studentData[0] || {};
      return { phone: (row.phone || "").trim(), address: (row.address || "").trim() };
    }
    return { phone: "", address: "" };
  }

  const fromState = state.students.find((item) => (item.guardian || "").toLowerCase() === normalizedName.toLowerCase());
  if (fromState) {
    return { phone: (fromState.phone || "").trim(), address: (fromState.address || "").trim() };
  }
  return { phone: "", address: "" };
}
...
async function linkGuardianToStudent(studentId, guardianName) {
  ...
  const normalizedGuardianName = formatPersonName(guardianName);
  if (!guardianId && normalizedGuardianName) {
    const { data } = await supabaseClient
      .from("profiles")
      .select("id")
      .ilike("name", normalizedGuardianName)
      .limit(1);
...
async function handleManualCheckin(studentId, options = {}) {
  ...
  showLabel(student, room, record, { autoPrint: true, openPreview: false });
  render();
...
function enqueueLabelPrint(job) {
  labelPrintQueue.push(job);
  processLabelPrintQueue();
}

async function processLabelPrintQueue() {
  if (labelPrintRunning || !labelPrintQueue.length) {
    return;
  }
  labelPrintRunning = true;
  while (labelPrintQueue.length) {
    const nextJob = labelPrintQueue.shift();
    await printLabelFromQueue(nextJob);
  }
  labelPrintRunning = false;
}

async function printLabelFromQueue(job) {
  const sent = await sendToLocalPrintService({
    checkinId: job?.checkinId || uid(),
    tipo: job?.tipo || "print",
    labelHtml: job?.labelHtml || ""
  });
  if (sent) {
    printServiceErrorNotified = false;
    return;
  }
  if (!printServiceWarned) {
    printServiceWarned = true;
    console.warn("Servico de impressao offline. Etiqueta nao foi impressa.");
  }
  if (!printServiceErrorNotified) {
    printServiceErrorNotified = true;
    alert("Servico de impressao indisponivel. Inicie o servico local para imprimir sem popup.");
  }
}

async function sendToLocalPrintService({ checkinId, tipo, labelHtml }) {
  if (!labelHtml) {
    return false;
  }
  const payload = {
    checkin_id: checkinId,
    conteudo: buildLabelHtmlDocument(labelHtml),
...
function getAgeFromBirth(birth) {
  const parsedBirth = parseBirthInput(birth);
  if (!parsedBirth) {
    return null;
  }
